...
class Part:
    def __init__(self, height: float, width: float):
        ...
        self.width = width  # width in inch
    ...
    def create_ribbing(self, thickness):  # Before call check to verify there isRibbing
        self.ribbing = Ribbing(self.needle_size, self.RPI, thickness, self)

    def create_taper(self, taper_offset, hem_size, taper_style=None):  # Before call check to verify there isTaper
        if taper_style is None:
            self.taper = Taper(taper_offset=taper_offset, hem_size=hem_size, width=self.width, working_height=self.working_height, total_stitches=self.stitches, working_rows=self.working_rows)
        else:
            self.taper = Taper(taper_offset=taper_offset, hem_size=hem_size, width=self.width,
                               working_height=self.working_height, total_stitches=self.stitches, working_rows=self.working_rows, taper_style=taper_style)

    def create_neck(self, offset_width, offset_height, neck_depth):  # Before call check to verify there isNeck
        self.neck = Neck(offset_width, offset_height, neck_depth, self.width, self.height, self.stitches, self.rows)
